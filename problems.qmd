...
:::{#exr-eoq-candy}
あるキャンディ専門店では，毎月1300本のペロペロキャンディが販売されている．発注にかかる費用は1回あたり8ドル，ペロペロキャンディの仕入れ単価は1本あたり0.75ドル，保管費用は1本あたり月0.225ドルである．EOQモデルを用いて，以下の問いに答えよ．なお，時間単位は月を用いるものとする．

1. 最適発注量と最適サイクル期間を求めよ．なお，計算結果は小数点以下2桁まで示すこと．
2. ペロペロキャンディの発注量は整数でなければならないとする．この場合の最適発注量を求めよ．
:::
...
:::{#exr-newsvendor-boy}
新聞売り子が新聞を仕入れ，販売する問題を考える。新聞 1 部の在庫不足費用を $p = 0.7$，在庫超過費用を $h = 0.18$ とする。新聞の需要 $D$ は，正規分布 $N(50, 8^2)$ に従うとする。標準正規分布 $Z \sim N(0,1)$ の累積分布関数を $\Phi(z)$ とし，$\Phi(0.82) \approx 0.795$ を用いてよい．以下の問いに答えよ。

1. 最適発注量に対応するサービスレベルを求めよ．
2. 上で求めたサービスレベルが，どのような確率を意味しているかを説明せよ．
3. 最適発注量を求めよ．
:::
...
:::{#exr-ahp-car}
田中さんは，新しく自動車の購入を検討している。候補としてA車，B車，C車の三つがある。田中さんは，デザイン，安全性，価格の3つの評価基準で自動車を評価したいと考え，以下の一対比較表を作成した。AHPを用いて，以下の問いに答えよ。ただし，整合性のもたない一対比較行列については，幾何平均法を用いて重要度ベクトルを算出すること．計算結果は四捨五入して小数点以下2桁まで示すこと。
...
nx.draw_networkx_edge_labels(
    G, pos, edge_labels=edge_labels, label_pos=0.3, font_size=15, rotate=False
)
plt.savefig("longest_path_graph.pdf", bbox_inches="tight")
plt.show()
```
