...
# Plotting the costs
plt.plot(Q, order_cost, label="Kd/Q", color="blue")
plt.plot(Q, holding_cost, label="hQ/2", color="orange")
plt.plot(Q, average_cost, label="g(Q)", color="green", linewidth=2)
plt.axvline(optimal_Q, color="red", linestyle="--", label="Q*")
...
1. B社は、毎月300個の需要がある商品を取り扱っている。一回の発注量は600個とし、サイクル期間 $T$ を求めよ。
...
2. 省略
3. 省略
4. $Q^* = \sqrt{\frac{2Kd}{h}}$、$T^* = \sqrt{\frac{2K}{hd}}$ である。
:::
   
## 実装

1. Python で EOQ モデルの最適発注量を計算する関数 `eoq(K, d, h)` を実装せよ。