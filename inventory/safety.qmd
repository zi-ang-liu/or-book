...
在庫管理には，$(r, Q)$ 方策が用いられるとする。在庫量が発注点 $r$ 以下になったときに、発注量 $Q$ を発注する。この場合、発注点 $r$ と発注量 $Q$ を決定変数とし、在庫の**期待コスト**（expected cost）を最小化することを目的とする。
...
単位期間あたりの需要 $D$ が連続一様分布 $U(200, 300)$ に従うと仮定する。平均需要は $\mu = 250$ である。一回の発注量を $Q = 500$ とする。
