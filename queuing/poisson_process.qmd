...
<!-- :::{#exm-exp-basic}
$X$ をパラメータ $\lambda$ の指数分布に従う確率変数とする．$X$ は客の到着間隔を表すとき，$\mathbb{E}[X]$ は平均到着間隔を表し，$\lambda$ は単位時間あたりの到着する客数の平均を表す．すなわち，$\lambda$ は到着率である．$\mathbb{P}(X \leq t)$ は，時間 $t$ 以内に次の客が到着する確率を表す．
::: -->
...
for lam, ls in zip(lambda_params, line_styles):
    ...
    plt.plot(x, pdf, label=f"λ={lam}", linestyle=ls)
...
::::{#exm-memoryless-arrival}
ある店舗での客の到着間隔が指数分布に従うとする．到着率は $\lambda = 2$ 人/時間である．すでに30分待っているとき，さらに15分待った後に客が到着していない確率を求めよ．
