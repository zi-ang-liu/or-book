...
ここでは，確率過程の基礎的な概念について説明する．特に，待ち行列理論で重要な役割を果たす**指数分布**と**ポアソン過程**について解説する．前の章で説明した到着間隔とサービス時間が指数分布に従うと仮定している．
...
で与えられ，$X \sim \text{Exp}(\lambda)$ と表す。ここで $\lambda > 0$ は分布のパラメータである。
...
待ち行列理論において，到着とサービス時間が指数分布に従うと仮定することはよくある．確率変数 $T$ を到着間隔を表すことを考える．このとき，$P(T \leq t)$ は，時間 $t$ 以内に次の客が到着する確率を表す．$E(T)$ は，到着間隔の平均であり，到着という事象が発生するまでの平均時間を示す．$\lambda = 1/E(T)$ は，単位時間あたりの到着する客数の平均を表す．この $\lambda$ は，前の章で説明した**平均到着率**である．

同様に，確率変数 $T$ をサービス時間を表す場合は，$E(T)$ はサービス時間の平均を示し，$\mu = 1/E(T)$ は単位時間あたりに処理できる客数の平均を表す．この $\mu$ は，前の章で説明した**平均サービス率**である．
...
単位を時間に揃えると，$T \sim \text{Exp}(12)$ に従う．$P(T \leq 1/30)$ を計算する．
$$
...
1. 単位時間あたり処理できる客数の平均を求めよ．
2. 平均サービス時間 $E[T]$ を求めよ．
3. 3分以内にサービスが完了する確率を求めよ．

単位時間あたり処理できる客数の平均は $6$ である．すなわち，平均サービス率は $\mu = 6$ である．

平均サービス時間 $E[T] = 1/\mu$ であるから，平均サービス時間は次のように計算される．
$$
E[T] = \frac{1}{\mu} = \frac{1}{6} \text{ 時間} = 10 \text{ 分}
$$

以上から，$T \sim \text{Exp}(6)$ に従う．累積分布関数を用いて，$P(T \leq 0.05)$ を計算する．
$$
...
確率変数 $T$ が次の事象が発生するまでの時間と考える．このとき，無記憶性とは，すでに時間 $s$ が経過している場合（$T > s$）において，さらに時間 $t$ 経過するまでに事象が発生する確率 $P(T > s + t \mid T > s)$ が，最初から時間 $t$ 経過するまでに事象が発生する確率 $P(T > t)$ と等しいことを意味する．次の事象が発生する確率は，過去の経過時間に依存しない．
...
計数過程は，時刻 $t$ までに発生した事象の回数を表す．例として，客の到着数，機械の故障回数などが挙げられる．
...
$P(N(t + \Delta t) - N(t) = 1) = \lambda \Delta t + o(\Delta t)$ は，時間区間 $[t, t + \Delta t]$ に1回事象が発生する確率を表す．$\Delta t$ が十分小さいとき，この確率は $\lambda \Delta t$ で近似できる．

$P(N(t + \Delta t) - N(t) \geq 2) = o(\Delta t)$ は，時間区間 $[t, t + \Delta t]$ に2回以上の事象が発生する確率を表す．$\Delta t$ が十分小さいとき，この確率は無視できるほど小さい．

**独立増分**（independent increments）とは，任意の $0 \leq t_1 < t_2 < \cdots < t_n$ に対して，
$$
N(t_2) - N(t_1), N(t_3) - N(t_2), \ldots, N(t_n) - N(t_{n-1})
$$
が互いに独立であることを意味する．

**定常増分**（stationary increments）とは，$t > s$ に対して，$N(t) - N(s)$ と $N(t + h) - N(s + h)$ が同一の分布に従うことを意味する．
